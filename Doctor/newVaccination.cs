...
namespace Doctor
{
    public partial class newVaccination : UserControl
    {
        ...
        public int TimeSlot
        {
            get { return timeslot - 1; }
        }

        public newVaccination()
        {
            InitializeComponent();
            ...
            lbpatientvaccination.Font = new Font(pfc.Families[0], 16, FontStyle.Bold);
            lbpatientinformation.Font = new Font(pfc.Families[0], 14, FontStyle.Bold);

            calenderdate.SetDate(DateTime.Now);
            
            cbvaccinationtype.SelectedIndex = 0;
        ...
        private void calenderdate_MouseHover(object sender, EventArgs e){}
        ...
        private void cbvaccinationtype_Click(object sender, EventArgs e)
        {
            lberror.Visible = false;
        }
                    ...
                    else
                    {
                        try
                        {
                            ...
                            if (checkDate >= DateTime.Parse(DateTime.Now.ToString("dd-MMM-yyyy")))
                            {
                                ...
                                {
                                    if (cbvaccinationtype.SelectedIndex != 0)
                                    {
                                        if (rbmale.Checked == true)
                                            gender = "Male";
                                        if (rbfemale.Checked == true)
                                            gender = "Female";
                                        timeslot = cbtimeslot.SelectedIndex;
                                        time = cbtimeslot.SelectedItem.ToString();
                                        vaccination = cbvaccinationtype.SelectedItem.ToString();
                                        insertData();
                                        if (NewVaccinationEvent != null)
                                        {
                                            NewVaccinationEvent(null, null);
                                        }
                                        this.Visible = false;
                                    }
                                    else
                                    {
                                        lberror.Text = "* Please select a Vaccination type";
                                        lberror.Visible = true;
                                    }
                                }
                                else
        ...
        public void insertData()
        {
            ...
            try
            {
                if (con.State == ConnectionState.Closed)
                    con.Open();
                SqlCommand cmd = new SqlCommand("VaccinationInsert", con);
                cmd.CommandType = CommandType.StoredProcedure;
                ...
                cmd.Parameters.AddWithValue("@vaccinationtype", this.Vaccination);
                cmd.Parameters.AddWithValue("@vaccinationdate", this.Date);
