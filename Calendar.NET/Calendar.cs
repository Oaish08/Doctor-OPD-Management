...
namespace Calendar.NET
{
    ...
    public class Calendar : UserControl
    {
        ...
        public event EventHandler mydateEvent;

        ///<summary>
        /// Gets the select date
        /// </summary>
        public string mydate
        {
            get { return _clickedEvent.Date.ToString("dd-MMM-yyyy"); }
        }
            ...
            if (e.Button == MouseButtons.Left && AllowEditingEvents)
            {
                if (_calendarView == CalendarViews.Month)
                {
                    ...
                    for (int i = 0; i < num; i++)
                    {
                        ...
                        {
                            _clickedEvent = z;
                            //Debug.WriteLine(_clickedEvent.Date.ToString("dd-MMMM-yyyy"));
                            if(mydateEvent!=null)
                            {
                                mydateEvent(null, null);
                            }
        ...
        {
            _calendarDate = DateTime.Now;
            _clickedEvent.Date = _calendarDate;
            if (mydateEvent != null)
            {
                mydateEvent(null, null);
            }
        ...
        private void PresetHolidays()
        {
            ...
            var dailydate = new CustomEvent
            {
                Date = new DateTime(2001, 01, 01),
                RecurringFrequency = RecurringFrequencies.Daily,
                EventText = " ",
                EventFont = new Font("Arial", 30),
                EventColor = Color.FromArgb(251,251,251)
            };
            AddEvent(dailydate);
        }
            ...
            for (int y = 0; y < numWeeks; y++)
            {
                for (int x = 0; x < 7; x++)
                {
                    if (rogueDays == 0 && counter <= DateTime.DaysInMonth(_calendarDate.Year, _calendarDate.Month))
                    {
                        ...
                        {
                            g.FillRectangle(new SolidBrush(Color.FromArgb(251, 251, 251)), xStart, yStart, cellWidth, cellHeight);
                        }
                        ...
                        if (first == false)
                        {
                         ...
                         && counter == DateTime.Now.Day)
                            {
                                g.DrawString(
                                    _calendarDate.ToString("MMMM") + " " + counter.ToString(CultureInfo.InvariantCulture),
                        ...
                        else
                        {
                         ...
                         && counter == DateTime.Now.Day)
                            {
                                g.DrawString(_calendarDate.ToString("MMM") + " " + counter.ToString(CultureInfo.InvariantCulture), _todayFont, Brushes.DarkCyan, xStart + 5, yStart + 2);
                            }
                            else
                            {
                                g.DrawString(_calendarDate.ToString("MMM") + " " + counter.ToString(CultureInfo.InvariantCulture), _daysFont, Brushes.Black, xStart + 5, yStart + 2);
                            }
                        }
                        counter++;
                    }
